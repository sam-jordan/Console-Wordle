...
export default class Game {
    ...
    checkCorrectness(guess: string): Letter[] {
        ...
        for (let i = 0; i < guess.length; i++) {
            if (comparison[i] === guess[i]) {
                // Adds the correct character to the checkedGuess array
                checkedGuess.splice(i, 1, {character: guess[i], accuracy: 'correct'});
        ...
        for (let i = 0; i < guess.length; i++) {
            if (comparison[i] !== '0') {
                ...
                if (comparison.includes(guess[i])) {
                    ...
                    checkedGuess.splice(i, 1, {character: guess[i], accuracy: 'incorrect'});
    ...
    checkGameOver(guessNumber: number): 0 | 1 | 2 {
        ...
        if (guessNumber > 1 && latestGuessAccuracy.every(val => val === 'correct')) {
            return 1
        }
        // If the user has had 6 guesses (and didn't guess a correct answer on the 6th), they lose
        if (guessNumber >= 6 && !latestGuessAccuracy.every(val => val === 'correct')) {
            return 2
        }
        return 0;
    }
